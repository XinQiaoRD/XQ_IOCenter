...
Server.connect = function(url, fn, download){
    ...
    if(!Url.web) {
        ...
        $.ajax({
            ...
            error:function(){
                console.log("数据服务器连接失败！启动临时应急模式！", url);
				Base = {};
                    console.log("Base准备完毕" , Base);
                    fn();
...
Server2.connect = function(url, fn, download){

    if(!Url.web) {
        FS = require('fs');
        http = require('http');

        $.getJSON("../uploads/password.json", function(data) {

            if(download===0){
                Base_password = data;
                console.log("Base准备完毕" , Base_password);
                fn();
            }else{
                console.log(url)
                $.ajax({
...
console.log(url);
        $.ajax({
            ...
            success: function(data){
                console.log("数据服务器连接成功！", url);
                Base_password = data;
                console.log("password_Base准备完毕" , Base_password);
                fn();
            },
            error:function(){
                console.log("数据服务器连接失败！启动临时应急模式！", url);

                $.getJSON("../uploads/password.json", function(json) {
                    Base_password = json;
                    console.log("password_Base准备完毕" , Base_password);
                    fn();
                });
